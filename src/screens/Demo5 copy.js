...
function Demo5() {
    ...
    const mappingOfferPeerConnection = (srcPeerUserId, localScreenSharing, remoteScreenSharing, peerConnection, dataChannel) => {
        ...
        if (!localScreenSharing && !remoteScreenSharing) {
            ...
            currentMapPeers = mapPeers.current.set(srcPeerUserId + ' Screen', [peerConnection, dataChannel])
    ...
    const createAnswer = async (offer, peerUserId, localScreenSharing, remoteScreenSharing, receiver_channel_name) => {
        ...
        console.log("Answer add local start");
        // Add local media stream to RTCPeerConnection
        addLocalTracks(peerConnection, localScreenSharing);
        console.log("Answer add local finish");
    ...
    const handleMessage = (event) => {
        ...
        else if (action == "stop-share-screen") {
    ...
    const startScreenShare = () => {
        mediaDevices.getDisplayMedia({
            ...
            .then(stream => {
                ...
                console.log("Get local display stream ", stream)
                if (localStream) {
                    localStream.getTracks().forEach(track => {
                        track.enabled = false;
                    });
                ...
                localDisplayStream.current = newStream;
