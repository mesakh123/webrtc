...
function Demo6() {
    ...
    const addLocalTracks = (peer, localScreenSharing) => {
        ...
        console.log("addLocalTracks localDisplayStream", localDisplayStream.current);
    ...
    const mappingOfferPeerConnection = (srcPeerUserId, localScreenSharing, remoteScreenSharing, peerConnection, dataChannel) => {
        ...
        if (!localScreenSharing && !remoteScreenSharing) {
            ...
            currentMapPeers = mapPeers.current.set(srcPeerUserId, [peerConnection, dataChannel])
            peerConnection.oniceconnectionstatechange = () => {
                ...
                if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed") {
                    ...
                    var newRemoteStreams = new Map(remoteStreams);
            ...
            currentMapPeers = mapPeers.current.set(newMapPeersKey, [peerConnection, dataChannel])
            var newSrcPeerUserId = srcPeerUserId + '-screen';
            setOnTrack(peerConnection, newSrcPeerUserId);
            peerConnection.oniceconnectionstatechange = () => {
                var iceConnectionState = peerConnection.iceConnectionState;
                if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed") {
    ...
    const setOnTrack = async (peer, srcPeerUserId) => {
        var mediaStream = new MediaStream();

        var newRemoteStreams = new Map(remoteStreams);
        newRemoteStreams = newRemoteStreams.set(srcPeerUserId, mediaStream);
        setRemoteStreams(newRemoteStreams);
        ...
        peer.onaddstream = event => {
            remoteStreams.set(srcPeerUserId, event.stream);
        ...
        if (!localScreenSharing && !remoteScreenSharing) {
            ...
            peerConnection.oniceconnectionstatechange = () => {
                ...
                if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed") {
                    ...
                    remoteStreams.delete(srcPeerUserId);
                }
            };
            mapPeers.current = new Map(currentMapPeers)
            ...
            peerConnection.ondatachannel = e => {
                ...
                peerConnection.dc.onmessage = evt => {
                    console.log('New message from %s\'s screen: ', newMapPeersKey, evt.data);
                }
                ...
                // otherwise, peer.dc may be undefined
                // as peer.ondatachannel would not be called yet
                currentMapPeers = currentMapPeers.set(newMapPeersKey, [peerConnection, peerConnection.dc]);
    ...
    const startScreenShare = () => {
        mediaDevices.getDisplayMedia({ audio: false, video: true })
            .then(stream => {
        ...
        <View style={[styles.flex, styles.rootContainer]}>
            ...
            <SafeAreaView style={[styles.flex, styles.rootContainer]}>
                <KeyboardAvoidingView
                    ...
                    <View style={[styles.flex, styles.shrinkingContainer]}>
                        {
