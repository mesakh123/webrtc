...
function MyGroupComponent() {
  ...
  return (
    <div>
      ...
      </button> <button onClick={toggleVideo}>
        {videoEnabled ? "Stop" : "Start"} Video
      </button> <button onClick={startScreenShare}>Start Screen Share</button> <button
      ...
      >
        ...
        <video key={index} srcObject={stream.stream} autoPlay={true} />
      ))}{" "}
    </div>
  );
}